...
class Smsc
{
    ...
    private $login = SMSC_LOGIN;        // логин клиента
    private $password = SMSC_PASSWORD;  // пароль
    private $methodPost = SMSC_POST;    // использовать метод POST
    private $useHttps = SMSC_HTTPS;
    private $charset = SMSC_CHARSET;    // кодировка сообщения: utf-8, koi8-r или windows-1251 (по умолчанию)
    private $debug = SMSC_DEBUG;        // флаг отладки
    private $from = SMTP_FROM;          // e-mail адрес отправителя

    public function __construct()
    {
        throw  new \Exception('Base properties defined.');
    }
...
// либо массив (0, -<код ошибки>) в случае ошибки
    ...
    public function get_status($id, $phone, $all = 0)
    {
        ...
        if (!strpos($id, ",")) {
            if ($this->debug) {
                if ($m[1] != "" && $m[1] >= 0) {
                    echo "Статус SMS = $m[0]", $m[1] ? ", время изменения статуса - ".date("d.m.Y H:i:s",
                            $m[1]) : "", "\n";
                } else {
                    echo "Ошибка №", -$m[1], "\n";
                }
...
// Функция вызова запроса. Формирует URL и делает 5 попыток чтения через разные подключения к сервису
    ...
    {
        $url = $_url = ($this->useHttps ? "https" : "http")."://smsc.ru/sys/$cmd.php?login=".urlencode($this->login)."&psw=".urlencode($this->password)."&fmt=1&charset=".$this->charset."&".$arg;
